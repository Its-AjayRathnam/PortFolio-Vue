...
var _TreeNodeValueBase = (_class = class {
  ...
  get path() {
    ...
    return (!parentPath || parentPath === "/") && pathSegment === "" ? "/" : _chunkXHAGIRMEjs.joinPath.call(void 0, parentPath || "", pathSegment);
  }
  toString() {
  ...
  get overrides() {
    return [...this._overrides.entries()].sort(
      ...
      return _chunkXHAGIRMEjs.mergeRouteRecordOverride.call(void 0, acc, routeBlock);
    }, {});
  }
  ...
  mergeOverride(filePath, routeBlock) {
    ...
    this._overrides.set(
      filePath,
      _chunkXHAGIRMEjs.mergeRouteRecordOverride.call(void 0, existing, routeBlock)
    );
  }
  /**
...
function splitFilePath(filePath, options) {
  ...
  let segment = head;
  if (!tail) {
    segment = _chunkXHAGIRMEjs.trimExtension.call(void 0, head, options.extensions);
  }
...
async function getRouteBlock(path, options) {
  const content = await _promises2.default.readFile(path, "utf8");
  const parsedSFC = await _compilersfc.parse.call(void 0, content, { pad: "space" }).descriptor;
  ...
  if (result) {
    if (result.path != null && !result.path.startsWith("/")) {
      _chunkXHAGIRMEjs.warn.call(void 0, `Overridden path must start with "/". Found in "${path}".`);
    }
  }
  return result;
}
function parseCustomBlock(block, filePath, options) {
  ...
  if (lang === "json5") {
    try {
      ...
      _chunkXHAGIRMEjs.warn.call(void 0, 
...
var RoutesFolderWatcher = class {
  ...
  on(event, handler) {
    this.watcher.on(event, (filePath) => {
      ...
      handler({
        filePath,
        routePath: _chunkXHAGIRMEjs.asRoutePath.call(void 0, { src: this.src, path: this.path }, filePath)
      });
    });
    return this;
  }
...
function resolveFolderOptions(globalOptions, folderOptions) {
  ...
  return {
    ...
    pattern: _chunkXHAGIRMEjs.appendExtensionListToPattern.call(void 0, 
      filePatterns,
...
function generateVueRouterProxy(routesModule, options, { addPiniaColada }) {
  return `
...
function extractDefinePageNameAndPath(sfcCode, id) {
  ...
  const setupAst = getSetupAst();
  const definePageNodes = (_nullishCoalesce(_optionalChain([setupAst, 'optionalAccess', _12 => _12.body]), () => ( []))).map((node) => {
    if (node.type === "ExpressionStatement")
      node = node.expression;
    return _common.isCallOf.call(void 0, node, MACRO_DEFINE_PAGE) ? node : null;
  }).filter((node) => !!node);
    ...
    if (prop.type === "ObjectProperty" && prop.key.type === "Identifier") {
      if (prop.key.name === "name") {
        if (prop.value.type !== "StringLiteral") {
          _chunkXHAGIRMEjs.warn.call(void 0, `route name must be a string literal. Found in "${id}".`);
        } else {
          routeInfo.name = prop.value.value;
        }
      } else if (prop.key.name === "path") {
        if (prop.value.type !== "StringLiteral") {
          _chunkXHAGIRMEjs.warn.call(void 0, `route path must be a string literal. Found in "${id}".`);
        } else {
...
function createRoutesContext(options) {
  ...
  async function scanPages(startWatchers = true) {
        ...
        return _fastglob2.default.call(void 0, folder.pattern, {
          ...
          (files) => Promise.all(
            files.map((file) => _pathe.resolve.call(void 0, folder.src, file)).map(
              (file) => addPage({
                routePath: _chunkXHAGIRMEjs.asRoutePath.call(void 0, folder, file),
                filePath: file
              })
            )
          )
    ...
    for (const route of editableRoutes) {
      await _optionalChain([options, 'access', _13 => _13.extendRoute, 'optionalCall', _14 => _14(route)]);
    }
  ...
  async function addPage({ filePath, routePath }, triggerExtendRoute = false) {
    ...
    if (triggerExtendRoute) {
      await _optionalChain([options, 'access', _15 => _15.extendRoute, 'optionalCall', _16 => _16(new EditableTreeNode(node))]);
    }
  }
  async function updatePage({ filePath, routePath }) {
    ...
    await writeRouteInfoToNode(node, filePath);
    await _optionalChain([options, 'access', _17 => _17.extendRoute, 'optionalCall', _18 => _18(new EditableTreeNode(node))]);
  }
  ...
  function generateRoutes() {
    const importsMap = new (0, _chunkXHAGIRMEjs.ImportsMap)();
...
`;
  ...
  async function _writeConfigFiles() {
    ...
    _chunkXHAGIRMEjs.logTree.call(void 0, routeTree, logger.log);
    if (dts) {
      ...
      if (lastDTS !== content) {
        ...
        lastDTS = content;
        _optionalChain([server, 'optionalAccess', _19 => _19.invalidate, 'call', _20 => _20(MODULE_ROUTES_PATH)]);
        _optionalChain([server, 'optionalAccess', _21 => _21.invalidate, 'call', _22 => _22(MODULE_VUE_ROUTER)]);
        _optionalChain([server, 'optionalAccess', _23 => _23.reload, 'call', _24 => _24()]);
      }
    }
    logger.timeEnd("writeConfigFiles");
  }
  const writeConfigFiles = _chunkXHAGIRMEjs.throttle.call(void 0, _writeConfigFiles, 500, 100);
...
var src_default = _unplugin.createUnplugin.call(void 0, (opt = {}, _meta) => {
  const options = _chunkXHAGIRMEjs.resolveOptions.call(void 0, opt);
  ...
  const filterPageComponents = _pluginutils.createFilter.call(void 0, 
    [
      ...options.routesFolder.map(
        (routeOption) => _pathe.join.call(void 0, routeOption.src, pageFilePattern)
      ),
